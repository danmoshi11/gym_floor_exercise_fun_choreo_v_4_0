...
// 精选体操音乐列表（从 Config 读取）
const BUILTIN_MUSIC = Config?.musicLibrary || [];
...
// 播放精选音乐 - 改用文件选择器避免CORS问题
window.playBuiltinMusic = function(index) {
    const music = BUILTIN_MUSIC[index];
    if (!music) return;
    
    // 提示用户选择对应的音频文件
    ToastManager.show('info', '选择音乐文件', `请选择 ${music.name} 对应的音频文件`, 3000);
    ...
    input.onchange = (e) => {
        ...
        if (audioFile) {
            ...
            MusicPlayer.play(blobUrl, music.name, `${flagEmoji} ${music.artist}`);
        }
    };
    input.click();
};
